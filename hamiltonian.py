    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        for j in range(n_orbs):
            for k in range(n_orbs):
                jk=j*n_orbs*4+k*4
                new_coefs[jk]  =self.integrals[j][k]*creation_operators[j].coefs[0]*annihilation_operators[k].coefs[0]
                new_coefs[jk+1]=self.integrals[j][k]*creation_operators[j].coefs[0]*annihilation_operators[k].coefs[1]
                new_coefs[jk+2]=self.integrals[j][k]*creation_operators[j].coefs[1]*annihilation_operators[k].coefs[0]
                new_coefs[jk+3]=self.integrals[j][k]*creation_operators[j].coefs[1]*annihilation_operators[k].coefs[1]
                
                new_pauli_strings[jk],phase  =creation_operators[j].pauli_strings[0]*annihilation_operators[k].pauli_strings[0]
                new_coefs[jk]  =new_coefs[jk]  *phase
                new_pauli_strings[jk+1],phase=creation_operators[j].pauli_strings[0]*annihilation_operators[k].pauli_strings[1]
                new_coefs[jk+1]=new_coefs[jk+1]*phase
                new_pauli_strings[jk+2],phase=creation_operators[j].pauli_strings[1]*annihilation_operators[k].pauli_strings[0]
                new_coefs[jk+2]=new_coefs[jk+2]*phase
                new_pauli_strings[jk+3],phase=creation_operators[j].pauli_strings[1]*annihilation_operators[k].pauli_strings[1]
                new_coefs[jk+3]=new_coefs[jk+3]*phase
...
class TwoBodyFermionicHamiltonian(FermionicHamiltonian):
    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        # TO COMPLETE (after lecture on mapping)
        count=0
        for j1 in range(n_orbs):  # creation 1
            for j2 in range(n_orbs):  # creation 2
                for j3 in range(n_orbs):  # annihilation 1
                    for j4 in range(n_orbs):  # annihilation 2
                        for k1 in range(2):
                            for k2 in range(2):
                                for k3 in range(2):
                                    for k4 in range(2):
                                        new_pauli_strings[count] = creation_operators[j1].pauli_strings[k1]
                                        new_pauli_strings[count],phase1 = new_pauli_strings[count]*creation_operators[j2].pauli_strings[k2]
                                        new_pauli_strings[count],phase2 = new_pauli_strings[count]*annihilation_operators[j3].pauli_strings[k3]
                                        new_pauli_strings[count],phase3 = new_pauli_strings[count]*annihilation_operators[j4].pauli_strings[k4]
                                        
                                        new_coefs[count]=0.5*phase1*phase2*phase3*self.integrals[j1][j2][j3][j4] * \
                                            creation_operators[j1].coefs[k1]*creation_operators[j2].coefs[k2] * \
                                            annihilation_operators[j3].coefs[k3]*annihilation_operators[j4].coefs[k4]
                                        count=count+1
        lcps = LinearCombinaisonPauliString(new_coefs,new_pauli_strings)
        ################################################################################################################
...
class MolecularFermionicHamiltonian(FermionicHamiltonian):
    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        # YOUR CODE HERE
        # TO COMPLETE (after lecture on mapping)
        pauli_one=self.one_body.to_linear_combinaison_pauli_string(creation_operators, annihilation_operators)
        pauli_two=self.two_body.to_linear_combinaison_pauli_string(creation_operators, annihilation_operators)
        out=pauli_one+pauli_two
        ################################################################################################################

...
class JordanWigner(Mapping):
    ...
    def fermionic_creation_annihilation_operators(self, n_qubits: int) -> tuple[list[LinearCombinaisonPauliString],
        ...
        for j in range(n_qubits):
            zx_R=np.zeros(2*n_qubits,dtype=np.bool_)
            zx_I=np.zeros(2*n_qubits,dtype=np.bool_)
            for k in range(n_qubits):
                ...
                if k==j:
                    zx_I[k]=True
                    zx_R[k+n_qubits]=True
                    zx_I[k+n_qubits]=True
            creation_operators.append(    0.5*PauliString.from_zx_bits(zx_R)+(-0.5)*1j*PauliString.from_zx_bits(zx_I))
            annihilation_operators.append(0.5*PauliString.from_zx_bits(zx_R)+  0.5 *1j*PauliString.from_zx_bits(zx_I))
        ################################################################################################################

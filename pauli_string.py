...
class PauliString:
    ...
    def from_zx_bits(cls, zx_bits: NDArray[np.bool_]) -> 'PauliString':
        ...
        n_zx=len(zx_bits)
        n_zx2=round(n_zx/2)
        x_bits=np.empty(n_zx2,dtype=bool)
        z_bits=np.empty(n_zx2,dtype=bool)
        for j in range(n_zx2):
            z_bits[j]=zx_bits[j]
            x_bits[j]=zx_bits[j+n_zx2]
        ################################################################################################################
    ...
    def from_str(cls, pauli_str: str) -> 'PauliString':
        ...
        ################################################################################################################
        n_ops=len(pauli_str)
        x_bits=np.empty(n_ops,dtype=bool)
        z_bits=np.empty(n_ops,dtype=bool)
        for j in range(n_ops):
            x_bits[j]=pauli_str[n_ops-j-1]=="X" or pauli_str[n_ops-j-1]=="Y"
            z_bits[j]=pauli_str[n_ops-j-1]=="Z" or pauli_str[n_ops-j-1]=="Y"
        ################################################################################################################
    ...
    def mul_pauli_string(self, other: 'PauliString') -> 'PauliString':
        ...
        ################################################################################################################
        new_x_bits=np.empty(len(self.x_bits),dtype=bool)
        new_z_bits=np.empty(len(self.x_bits),dtype=bool)
        phase=complex(1.0,0.0)
        for j in range(len(self.x_bits)):
            new_x_bits[j] = (self.x_bits[j] or other.x_bits[j]) and ~(self.x_bits[j] and other.x_bits[j])
            new_z_bits[j] = (self.z_bits[j] or other.z_bits[j]) and ~(self.z_bits[j] and other.z_bits[j])
            if   ( self.x_bits[j] and ~self.z_bits[j]) and ((~other.x_bits[j] and  other.z_bits[j])):
                phase=phase*(-1j)
            elif (~self.x_bits[j] and  self.z_bits[j]) and (( other.x_bits[j] and ~other.z_bits[j])):
                phase=phase*(1j)
            elif ( self.x_bits[j] and ~self.z_bits[j]) and (( other.x_bits[j] and  other.z_bits[j])):
                phase=phase*(1j)
            elif ( self.x_bits[j] and  self.z_bits[j]) and (( other.x_bits[j] and ~other.z_bits[j])):
    ...
    def ids(self) -> NDArray[np.bool_]:
        ...
        ################################################################################################################
        ids=np.empty(len(self.x_bits),dtype=bool)
        for j in range(len(self.x_bits)):
            ids[j]=~self.x_bits[j] and ~self.z_bits[j]
        ################################################################################################################
...
class LinearCombinaisonPauliString:
    ...
    def mul_linear_combinaison_pauli_string(self, other: 'LinearCombinaisonPauliString') -> 'LinearCombinaisonPauliString':
        ...
        for j in range(len(self)):
            for k in range(len(other)):
                new_str,phase=self.pauli_strings[j]*other.pauli_strings[k]
                new_pauli_strings[j*len(other)+k]=new_str
                new_coefs[j*len(other)+k]=phase*self.coefs[j]*other.coefs[k]
        ################################################################################################################
    ...
    def to_zx_bits(self) -> NDArray[np.bool_]:
        ...
        ################################################################################################################
        for j in range(len(self)):
            zx_bits[j]=self.pauli_strings[j].to_zx_bits()
        ################################################################################################################
    ...
    def to_xz_bits(self) -> NDArray[np.bool_]:
        ...
        ################################################################################################################
        for j in range(len(self)):
            xz_bits[j]=self.pauli_strings[j].to_xz_bits()
        ################################################################################################################
    ...
    def ids(self) -> NDArray[np.bool_]:
        ...
        for j in range(len(self)):
            for k in range(self.n_qubits):
                ids[j][k]=~(self.pauli_strings[j].x_bits[k] or self.pauli_strings[j].z_bits[k])
        ################################################################################################################
    ...
    def combine(self) -> 'LinearCombinaisonPauliString':
        ...
        pauli_strings_zx=self.to_zx_bits()
        unique_zx=np.unique(pauli_strings_zx,axis=0)
        new_coefs=np.zeros(len(unique_zx),dtype=np.complex_)
        new_pauli_strings=np.zeros(len(unique_zx),dtype=PauliString)
        for k in range(len(unique_zx)):
            new_pauli_strings[k]=PauliString.from_zx_bits(unique_zx[k])
            for l in range(len(pauli_strings_zx)):
                if (unique_zx[k]==pauli_strings_zx[l]).all():
                    new_coefs[k]=new_coefs[k]+self.coefs[l]
        ################################################################################################################
    ...
    def apply_threshold(self, threshold: float = 1e-6) -> 'LinearCombinaisonPauliString':
        ...
        ################################################################################################################
        new_pauli_strings=np.zeros(0,dtype=PauliString)
        new_coefs=np.zeros(0,dtype=np.complex_)
        for j in range(len(self)):
            if np.abs(self.coefs[j])>=threshold:
                new_pauli_strings=np.append(new_pauli_strings,[self.pauli_strings[j]],axis=0)
                new_coefs=np.append(new_coefs,self.coefs[j])
        ################################################################################################################
    ...
    def sort(self) -> 'LinearCombinaisonPauliString':
        ...
        int_ar=np.zeros(len(zx_bits),dtype=np.int32)
        for j in range(len(self)):
            for k in range(len(zx_bits[j])):
                int_ar[j]=int_ar[j]+zx_bits[j][k]*2**k
        order = np.argsort(int_ar)
        #order = np.argsort(np.sum(self.to_zx_bits(),axis=1))
        ################################################################################################################
